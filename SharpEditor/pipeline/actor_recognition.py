...
class ActorRecognizer:
    ...
    def _cluster_actors(self, embeddings: np.ndarray) -> np.ndarray:
        ...
        Uses precomputed cosine distance for robustness to lighting and
        angle variation across shots. On L2-normalized embeddings, cosine
        distance = 1 - cosine_similarity, which ranges from 0 (identical)
        to 2 (opposite).
        """
        ...
        sim_matrix = cos_sim(embeddings)  # [N, N] in [-1, 1]
        distance_matrix = np.clip(1.0 - sim_matrix, 0.0, 2.0).astype(np.float64)
    ...
    def process_dataset(
        ...
        *,
        ...
        if not all_face_paths:
            # Write empty actors.json for every shot so downstream stages
            # always find the file.
        ...
        # Finalize actor-level mean embeddings (L2-normalized) and export exemplars
        actors_export_dir = embeddings_root_path / "actors"
        if not actors_export_dir.exists():
            actors_export_dir.mkdir(parents=True, exist_ok=True)
        ...
        for actor_id, entry in actors.items():
            ...
            # Export up to 5 exemplars for visual inspection
            actor_dir = actors_export_dir / actor_id
            ...
            # Select evenly spaced faces to get variety 
            num_faces = len(faces)
            if num_faces > 0:
                max_exemplars = min(5, num_faces)
                indices = np.linspace(0, num_faces - 1, max_exemplars, dtype=int)
                for i, idx in enumerate(indices):
                    src_path = Path(faces[idx]["face_image"])
                    if src_path.exists():
                        # name like actor_000_00.jpg
                        dst_path = actor_dir / f"{actor_id}_{i:02d}{src_path.suffix}"
                        shutil.copy2(str(src_path), str(dst_path))
                        exemplars.append(str(dst_path))
        ...
        # Write empty actors.json for shots that had zero detected faces
        # so downstream stages always find the file.
        for shot_dir in shot_dirs:
            if shot_dir.name not in per_shot:
                out_path = shot_dir / "actors.json"
                if out_path.exists() and not overwrite:
                    continue
                write_json(out_path, {
